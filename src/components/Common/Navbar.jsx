        ...
        <main>
            <nav className="flex justify-between px-4 sm:px-8 items-center py-4">
                <div className="flex items-center gap-4">
                    ...
                    <Link to="/">
                        <img src={LordImage} alt="Logo" className=" h-20 aspect-auto sm:h-18 hidden sm:block" />
                    </Link>
                    {navLinks.map((link, key) => (
                        <Link
                            to={link.href}
                            key={key}
                            className={`font-mono hover:text-custom-header-bg underline-animate hidden sm:block ${
                                location.pathname === link.href ? "text-lime-500 after:w-4" : "text-black-400"
                            }`}
                        >
                            {link.label}
                        </Link>
                    ...
                    {/* {localStorage.getItem("super_user") === "true" && (
                        <Link
                            ...
                            key="view-users"
                            className={`font-mono hover:text-custom-header-bg underline-animate hidden sm:block ${
                                location.pathname === "/view-users" ? "text-lime-500 after:w-4" : "text-black-400"
                            }`}
                        >
                            Users
                        </Link>
                ...
                >
                    <section className="text-black w-64 bg-white flex flex-col absolute left-0 top-0 h-screen p-8 gap-8 z-50">
                        ...
                        {localStorage.getItem("super_user") === "true" && (
                            <Link to="/view-users" key="view-users-mobile" className="font-mono text-black-900" onClick={() => setMenu(false)}>
                                Users
                            </Link>
                        )}
                    </section>
                ...
                <section className="flex flex-col sm:flex-row items-center gap-1 sm:gap-4 font-mono">
                    ...
                    <div className="flex flex-col gap-1 text-center sm:text-left">
                        <div className="">
                            ...
                            {localStorage.getItem("super_user") === "true" && (
                                <span className="font-mono text-green-700 text-sm rounded-sm bg-zinc-200 w-min px-2 py-px mx-2 font-semibold">Admin</span>
                            )}
                        </div>
                        <span className="sm:text-base text-xs text-red-600 font-semibold ">
                            {localStorage.getItem("user_name")},{" "}
                            {localStorage.getItem("is_mla") === "true" ? "MLA" : localStorage.getItem("is_mla") === "false" ? "MP" : "Admin"}
                        </span>
                        <span className=" text-sm text-zinc-700 font-bold ">{localStorage.getItem("constituency")} Constituency</span>
                    </div>
                            ...
                            <div className="absolute right-0 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-50" ref={userSettingsRef}>
                                <Link
                                    ...
                                    className={`block px-4 py-2 text-gray-800 hover:bg-gray-100 ${
                                        location.pathname === "/blockdates" ? "text-red-500 font-semibold" : "text-gray-700"
                                    }`}
                                >
                                    Block/Unblock
                                </Link>
                                {localStorage.getItem("super_user") === "true" && (
                                    <Link
                                        to="/add_newuser"
                                        className={`block px-4 py-2 text-gray-800 hover:bg-gray-100 ${
                                            location.pathname === "/add_newuser" ? "text-red-500 font-semibold" : "text-gray-700"
                                        }`}
                                    >
                                        Add New-User
                                    </Link>
