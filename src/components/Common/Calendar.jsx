...
const Calendar = () => {
  ...
  const getBlockedDates = async () => {
    try {
      ...
      const dates = blockedDatesResponse?.data?.Blocked.map((item) =>
        format(new Date(item.blockdate), "yyyy-MM-dd")
      );
  ...
  const isCurrentMonth = () => {
    const today = startOfToday();
    return (
      currentPageStart.getFullYear() === today.getFullYear() &&
      currentPageStart.getMonth() === today.getMonth()
    );
  };
  useEffect(() => {
  ...
  const getMonthSlotAvailability = async () => {
    try {
      const res = await apis.getMonthSlotAvailability(
        currentPageStart.getMonth() + 1,
        currentPageStart.getFullYear()
      );
      if (res?.data) {
      ...
      <div className={`flex justify-center mb-4`}>
        {!isCurrentMonth() && (
          <button
            ...
            className="text-2xl text-black p-2 rounded hover:text-custom-header-bg"
          >
            <FiArrowLeft />
          </button>
        )}
              ...
              <div
                className={`w-18 h-14 sm:h-16 sm:w-18 text-center rounded-lg text-black border    ${
                  isDayBeforeToday(day)
                    ? "bg-slate-500 md:bg-slate-300 md:hover:bg-slate-400"
                    : "hover:bg-slate-200"
                }
                   ...
                   ${
                     is768
                       ? isDayBeforeToday(day)
                         ...
                         : getDayClassforSmallScreens(day)
                       : ""
                   }
                   border-gray-300 mb-2 md:grid md:grid-cols-12 items-center `}
                onClick={() =>
                  !isDayBeforeToday(day) &&
                  handleBooking(format(day, "yyyy-MM-dd"))
                }
                style={{
                  width: "auto",
                      ...
                      <div className="grid  text-center justify-center pl-6">
                        {
                          <div className="hidden md:block ">
                            <FaTicketAlt
                              size={25}
                              className={`${getDayClass(day)} text-base  `}
                            />
                          </div>
                        }
