    ...
    pilgrims: yup.array().of(
        yup.object({
            ...
            aadhaar_number: yup
                ...
                .test("len", "Aadhaar must be exactly 12 digits", (value) => value.toString().length === 12)
                .required("Aadhaar is required"),
            seva: yup.string().default("VIP Break"),
            booked_datetime: yup.string(),
        })
    ),
    pilgrim_reference: yup.string().notRequired(),
});
...
const AddEditFormLayout2 = ({
    ...
    const [disableAddPilgrimsButton, setDisableAddPilgrimsButton] = useState(false);
    ...
    useEffect(() => {
        if (Object.keys(payloadForDownload).length === 0) setStartPreviewDownload(false);
    }, [payloadForDownload]);
    ...
    const onSubmit = async (data) => {
        try {
            ...
            data.pilgrims.forEach((pilgrim) => {
                ...
                if ((pilgrim.clicked && pilgrim.editable == false) || (editMode && bookedPilgrimDetails.length > 0)) {
                    editPilgrims.push(pilgrim);
                }
                if (!Object.prototype.hasOwnProperty.call(pilgrim, "editable")) newPilgrims.push(pilgrim);
            });
    ...
    const handleDelete = (index) => {
        ...
        if (fields.length !== 1) setPilgrimCount(fields.length - 1);
    };

    const handlePilgrimDelete = async () => {
        try {
            const res = await apis.deletePilgrim(fields[deleteIndex].pilgrim_id);
            let date = format(parseISO(fields[deleteIndex]?.booked_datetime), "dd MMM yyyy");
            if (res.status == 204) {
                setToastMessage({
                    type: "success",
    ...
    const handlePreviewOrDownload = () => {
        ...
        setPayloadForDownload({
            ...
            accommodation_date: format(subDays(format(parseISO(bookedPilgrimDetails[0]?.booked_datetime), "yyyy-MM-dd"), 1), "yyyy-MM-dd"),
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="bg-white rounded-lg p-6 w-full max-w-8xl">
                ...
                <div>
                    {fields.map((field, index) => (
                        <div key={field.id} className="flex  gap-2 mb-4" style={{ gridTemplateColumns: "repeat(14, 1fr)" }}>
                            <div className="col-span-12 md:col-span-3 relative ml-2">
                                ...
                                />
                                <div className="absolute px-1 inset-y-0 left-0 flex items-center pointer-events-none ">
                            ...
                            <div className="md:col-span-2 col-span-6 relative">
                                ...
                                <div className="flex absolute items-center inset-y-0 left-1.5  space-x-2">
                                    <FaPhoneAlt className="text-gray-400 scale-95" />
                                </div>
                            ...
                            <div className="md:col-span-1 col-span-2 relative">
                                <input
                                    ...
                                    className={`border border-gray-300 rounded p-2 w-16 disable-spinner ${field.editable ? "bg-slate-200" : ""}`}
                                    disabled={field.editable} // Disable input based on editable flag
                                />
                                <div className="absolute pl-8 inset-y-0 left-0 flex items-center pointer-events-none h-9"></div>
                                {errors?.pilgrims?.[index]?.age && <p className="text-red-600 text-xs">{errors.pilgrims[index].age.message}</p>}
                            </div>

                            <div className="md:col-span-3 col-span-4">
                                <select
                                    {...register(`pilgrims.${index}.seva`)}
                                    className={`border border-gray-300 rounded p-2 w-full h-[2.62rem] ${field.editable ? "bg-slate-200" : ""}`}
                                    disabled={field.editable} // Disable input based on editable flag
                                ...
                                {errors?.pilgrims?.[index]?.seva && <p className="text-red-600 text-sm">{errors.pilgrims[index].seva.message}</p>}
                            </div>
                                    ...
                                    {bookedPilgrimDetails.some((pilgrim) => pilgrim.aadhaar_number === field.aadhaar_number) && (
                                        <div>
                                            {field.editable ? (
                                                ...
                                                <button type="button" className="text-black" onClick={() => toggleEdit(index)}>
                ...
                <div className="flex text-center justify-center">
                    {(fields.some((field) => field.clicked === true) || (editMode && bookedPilgrimDetails.length > 0)) && (
                        <button type="submit" className="bg-lime-500 text-white font-mono py-2 w-20 px-2 rounded hover:bg-lime-700">
                            Edit
                        </button>
                    )}
                </div>
