...
const AddEditFormLayout2 = ({ bookingsObject,getPilgrimDetails, getMonthSlotAvailability,date, bookingsCount, setIsModalOpen, setToastMessage, isModalOpen, bookedPilgrimDetails=[] }) => {
    ...
    if(editPilgrims.length>0)
      {
        ...
        const transformedObject = editPilgrims.map(obj=>({
            ...obj,"pilgrim_count":parseInt(pilgrimCount)
        }))
        ...
        if (response.status === 200) {
          ...
          setHideAddCountButton(false)
          setEditMode(false)
          setToastMessage({ type: 'success', message: `Details Successfully updated for date-${date}` });
          reset();
        }
      ...
      <MyModal isOpen={openModal} handlePilgrimDelete = { handlePilgrimDelete} setIsModalOpen={setOpenModal} title="Delete?" message="Are you sure to Delete the Booked Pilgrim"/>
       ...
       <label> Enter number of pilgrims
            <select onChange={e=>{
              ...
              setDisableAddPilgrimsButton(true)}} value={pilgrimCount} disabled={shouldDisableEditDeleteIn38hrs(bookedDateTime)}
...
{shouldDisableEditDeleteIn38hrs(bookedDateTime) && <div className="text-sm text-red-500 font-mono">Cannot edit before 36 hours of booking date</div>}
       ...
       &&
        ...
        <div>
          <button className=" px-4 py-2 rounded" onClick={()=>{
            ...
            { <PDFViewer payloadForDownload={payloadForDownload} setStartPreviewDownload={setStartPreviewDownload} download={startPreviewDownload} setPayloadForDownload={setPayloadForDownload}/>}
      ...
      {(hideAddCountButton||bookedPilgrimDetails.length>0) && <div className="tex-xs text-lime-500 font-mono">Pilgrim Detail(s):</div>}
{fields.map((field, index) =>(
  ...
  {
            ((fields.some(field=>field.clicked===true))|| (editMode && bookedPilgrimDetails.length>0)) && <button type="submit" className="bg-lime-500 text-white font-mono py-2 w-20 px-2 rounded hover:bg-lime-700" >
          Edit
          </button>
          }
    </div>  
...
<div >
        ...
        <div className={`flex text-center justify-center `}>
          {hideAddCountButton &&!bookedPilgrimDetails.length> 0&& <button type="submit" disabled={disableAddPilgrimsButton}  className={`${disableAddPilgrimsButton?'bg-transparent':'bg-lime-500 hover:bg-lime-700'} text-white font-mono text-xs py-3 w-20 px-1 rounded `}>
          AddPilgrims
          </button>
